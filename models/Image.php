...
class Image extends \app\models\BaseModel
{
    ...
    public $_icon_classes = [
        ...
        'avi' => 'bi-filetype-file',
        'mp4' => 'bi-filetype-file',
    ];
    ...
    {
        return $this->extension and in_array($this->extension, $this->_video_extensions);
    }
    ...
    public function getImg($width = 100, $height = 100)
    {
        if($extension = $this->extension) {
            ...
            elseif(in_array($extension, $this->_video_extensions)) {
                return "<video width='{$width}' height='{$height}'><source src='/upload/$this->path'></video>";
            }
            else {
    ...
    {
        if($this->extension) {
            $filePath = Yii::getAlias('@upload').'/views/icons/'.$this->extension.'.php';
            $viewName = 'file';
            if($this->isVideo) {
                $viewName = 'video';
            }
